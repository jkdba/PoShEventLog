## Get psd1 file
$RawPSD = Get-Content -Path "$($PSScriptRoot)\..\PoShEventLog.psd1"
...
$RawPSD | ForEach-Object { $_ -replace "(?<=^ModuleVersion = ')\d+\.\d\.\d\.\d(?=')",$NewModuleVersion } | Out-File -FilePath  "$($PSScriptRoot)\..\PoShEventLog.psd1" -Force
